...
class NewsController extends Controller {
    ...
    public function lists() {
        ...
        $list = $news->where("1=1")->order("updatedtime DESC")->limit($Page->firstRow . ',' . $Page->listRows)->select();
        if (isset($list)) {
    ...
    public function setcache(){
        ...
        S("key", $now, array('type' => 'file', 'expire' => 600));
    ...
    public function edit() {
        ...
        if (IS_POST) {
            ...
            $data['title']   = $title;
            $data['content'] = stripslashes(htmlspecialchars_decode($content));
            $data['author']  = $author;
            //$data['createdtime'] = date('Y-m-d H:i:s', time());
            $data['updatedtime'] = date('Y-m-d H:i:s', time());
            ...
            if ((int) $rlt > 0) {
                ...
                echo "<script>alert('编辑成功');window.location.href='" . $url . "';</script>";
                //redirect($url);
            } else {
                $this->display();
            }
